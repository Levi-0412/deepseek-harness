...
// Type-only: pulls this package's locale-namespace merge into the spec program.
 ...
 * runtime resolves them, and a shared `common` key falls back to the key text.
 */
const t: PropsLocale<'rdagent'>['t'] = (key, params) => {
  const template = (en as Record<string, string>)[key] ?? key
  if (params === undefined) return template
  return template.replace(/\{(\w+)\}/g, (match, name: string) => (name in params ? String(params[name]) : match))
}

/** Render the panel with the two props its trigger supplies in production. */
function renderPanel(): void {
  const props: RdagentPanelProps = { onClose: () => {}, t }
  render(<RdagentPanel {...props} />)
}

/** Render the footer action in its wide column, with the drawer closed. */
function renderTrigger(): void {
  const props: RdagentTriggerProps = { wide: true, t }
  render(<RdagentTrigger {...props} />)
}
